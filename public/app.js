...
function render(snapshot){
  ...
  root.innerHTML=visible.map(t=>{
    ...
    const preview=onlineSupported
      ...
      : '<div class="no-online">Online status unavailable for Rustoria</div>';
    return `<section class="team-card" data-team-id="${esc(t.id)}" tabindex="0" aria-label="Otwórz team ${esc(t.name)}">
      ...
      <div class="members-head"><span>${onlineSupported?'Online now':'Players'}</span><b>${onlineSupported?`${online.length}/${t.members.length} online`:`${t.members.length} tracked`}</b></div>
