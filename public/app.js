...
function render(snapshot){
  ...
  root.innerHTML=teamsMeta.map(meta=>{
    ...
    return `<section class="team-card"><div class="team-title"><h2>${esc(t.name)}</h2><span>${ago(snapshot.timestamp)}</span></div><div class="muted">Created ${new Date(t.createdAt).toLocaleDateString('pl-PL')}</div><div class="status-row"><span class="tracked">Tracked</span><span title="Stats">▥</span><button class="icon-btn team-edit" data-team-id="${esc(t.id)}" title="Team settings">⚙</button></div><div class="stats">${statDefs.map(([k,i,l])=>`<div class="stat"><div class="stat-label"><span>${i}</span>${l}</div><strong>${fmt(t.stats?.[k])}</strong></div>`).join('')}</div><div class="separator"></div><div class="members-head"><span>MEMBERS</span><b>${t.members.filter(m=>m.isOnline).length}/${t.members.length} online</b></div><div class="members">${t.members.map(m=>`<div class="member"><span class="dot ${m.isOnline?'online':'offline'}" title="${m.isOnline?'Online':'Offline'}"></span>${avatar(m)}<span class="${m.isOnline?'member-online':''}" title="${esc(m.steamId)} · ${m.isOnline?'ONLINE':'offline'}">${esc(m.name||m.steamId)}</span></div>`).join('')}</div><details><summary>Raid stats</summary><div class="raid">HV: ${fmt(t.stats?.hvRockets)} · C4: ${fmt(t.stats?.c4)} · Explo: ${fmt(t.stats?.explosiveAmmo)} · Satchels: ${fmt(t.stats?.satchels)} · Kills: ${fmt(t.stats?.kills)}</div></details></section>`;
