...
function render(snapshot){
  ...
  root.innerHTML=teamsMeta.map(meta=>{
    ...
    return `<section class="team-card" data-team-id="${esc(t.id)}" tabindex="0" aria-label="Otwórz team ${esc(t.name)}">
      ...
      <div class="stats">${statDefs.map(([k,i,l])=>`<div class="stat stat-watermark-card"><img class="stat-watermark-img" src="${i}" alt="" loading="lazy"><div class="stat-label"><span>${l}</span></div><strong>${fmt(t.stats?.[k])}</strong></div>`).join('')}</div>
